...
{
  std::cout << "Closing modeline signal listener" << std::endl;
}
...
void SignalClient::stop_work()
{
  ...
  std::cout << "Stopping modeline signal client" << std::endl;
  m_shall_stop = true;
}
...
void SignalClient::get_data(Glib::ustring * p_title) const
{
  Glib::Threads::Mutex::Lock lock(m_Mutex);
//   if(title.length() > 7)
    *p_title = title;
...
void SignalClient::listen(ModelineWin * p_caller)
{
  {
    Glib::Threads::Mutex::Lock lock(m_Mutex);
    caller = p_caller;
